...
kubectl --kubeconfig=${CLUSTER_NAME}.kubeconfig get gitrepository $REPO_NAME -n flux-system 2>/dev/null
if [ $? -eq 0 ]; then
...
SYNC_STATUS=$(kubectl --kubeconfig=${CLUSTER_NAME}.kubeconfig get gitrepository $REPO_NAME -n flux-system -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}')
