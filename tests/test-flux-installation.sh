...
kubectl --kubeconfig=${WORKLOAD_CLUSTER_NAME}.kubeconfig get namespace flux-system 2>/dev/null
if [ $? -eq 0 ]; then
...
FLUX_PODS=$(kubectl --kubeconfig=${WORKLOAD_CLUSTER_NAME}.kubeconfig get pods -n flux-system --field-selector=status.phase=Running --no-headers | wc -l)
...
# Test Flux readiness
flux check --kubeconfig=${WORKLOAD_CLUSTER_NAME}.kubeconfig 2>/dev/null
if [ $? -eq 0 ]; then
